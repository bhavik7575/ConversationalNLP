...
model=Word2Vec.load("updatedInsurance_word2vec_v3_18650_tri1")
#display_closestwords_tsnescatterplot(model, 'pay')

#vector building

features_dim=model.wv.syn0.shape[1]

vec = np.zeros(features_dim).reshape((1, features_dim))
train_vecs_w2v=vec
...
def get_Intent(utterence):
    ...
    print(utterence)
    query_vector = vectorize_query(utterence)
    scoretable = shuffle(datatable)

    resultTable = pd.DataFrame(columns=['Score'])
    for i in range(0, scoretable.__len__()):
        vec = (np.array(scoretable.loc[i, 1:]))
